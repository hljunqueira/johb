...
-- Relacionamento Produto-Complemento (vincula opcionais aos produtos)
...
CREATE TABLE IF NOT EXISTS orders (
    ...
    delivery_type VARCHAR(50) NOT NULL, -- 'entrega' ou 'retirada'
    address TEXT,
    ...
    status VARCHAR(50) DEFAULT 'aguardando', -- 'aguardando', 'preparando', 'entregue'
    payment_status VARCHAR(50) DEFAULT 'pendente', -- 'pendente', 'pago'
    observation TEXT,
    rating INTEGER,
    rating_comment TEXT,
...
-- ============================================
-- BANNERS TABLE (para HeroSection configurável)
-- ============================================
...
-- Triggers for updated_at on banners
...
-- ============================================
-- COMBOS TABLE (para ComboSuggestion configurável)
-- ============================================
...
-- Triggers for updated_at on combos
...
-- ============================================
-- COMBO ITEMS TABLE (itens que compõem cada combo)
-- ============================================
...
INSERT INTO banners (title, subtitle, image_url, cta_text, cta_link, active, "order") VALUES
    ...
    ('Combos Especiais', 'Economize até 20% com nossos combos exclusivos', 'https://images.unsplash.com/photo-1512621776951-a57141f2eefd?w=800', 'Ver combos', '#', TRUE, 1)
ON CONFLICT DO NOTHING;
...
BEGIN
    -- Get category IDs
    SELECT id INTO cat_saladas FROM categories WHERE name = 'Saladas';
    SELECT id INTO cat_sucos FROM categories WHERE name = 'Sucos Naturais';
    SELECT id INTO cat_sobremesas FROM categories WHERE name = 'Sobremesas';

    -- Insert Combo 1: Salada + Suco
    INSERT INTO combos (name, description, image_url, base_price, discount_percent, active, "order")
    VALUES ('Combo Leve', '1 Salada + 1 Suco Natural. Perfeito para o almoço saudável.', 'https://images.unsplash.com/photo-1512621776951-a57141f2eefd?w=400', 55.00, 10, TRUE, 0)
    RETURNING id INTO combo1_id;

    -- Insert Combo 2: Salada + Suco + Sobremesa
    INSERT INTO combos (name, description, image_url, base_price, discount_percent, active, "order")
    VALUES ('Combo Completo', '1 Salada + 1 Suco + 1 Sobremesa. A refeição completa!', 'https://images.unsplash.com/photo-1490645935967-10de6ba17061?w=400', 75.00, 15, TRUE, 1)
